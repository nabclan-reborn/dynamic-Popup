...
{
    return [self initWithOrigin:(CGPoint)origin radius:r lineWidth:1.2f];
}
...
- (instancetype)initWithOrigin:(CGPoint)origin radius:(CGFloat)r lineWidth:(CGFloat)width
{
    ...
    if (self) {
        ...
        _countLabel.font = [UIFont fontWithName: @"HelveticaNeue-Bold" size:8.2f];
...
- (void)updateFrame:(CGSize)size
{
    ...
    CGFloat originX = size.width - (2*r) - 4;
...
- (void)updateTimerButton:(NSTimer *)timer
{
    if (_reverse)
    {
        ...
        if(currentAngle >= (366 + START_DEGREE_OFFSET)) {
            [self stopTimer];
        }
    }
