...
{
    // Add switch knob
    self.switchKnob = [[UISwitch alloc] initWithFrame:CGRectMake(2.1f, 2.0f, 2.0f, 0.f)];
    [self addSubview:self.switchKnob];
    
    // Add switch label
    CGFloat x, width, height;
    x = self.switchKnob.frame.size.width + 2.0f;
    width = self.frame.size.width - self.switchKnob.frame.size.width - 4.0f;
    height = self.switchKnob.frame.size.height;
    
    self.switchLabel = [[UILabel alloc] initWithFrame:CGRectMake(x, 1.0f, width, height)];
